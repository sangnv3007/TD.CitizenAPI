...
public class CreateMarketProductRequestHandler : IRequestHandler<CreateHotlineRequest, Result<Guid>>
{
    ...
    {
        var item = new Hotline(request.Name, request.Address, request.Code, request.Detail, request.OtherDetail, request.Phone, request.Image, true, request.Order, request.HotlineCategoryId, request.Latitude, request.Longitude);
