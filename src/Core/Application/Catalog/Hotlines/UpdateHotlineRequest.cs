...
public class UpdateHotlineRequestHandler : IRequestHandler<UpdateHotlineRequest, Result<Guid>>
{
    ...
    public async Task<Result<Guid>> Handle(UpdateHotlineRequest request, CancellationToken cancellationToken)
    {
        ...
        item.Update(request.Name, request.Address, request.Code, request.Detail, request.OtherDetail, request.Phone, request.Image, request.Active, request.Order, request.HotlineCategoryId, request.Latitude, request.Longitude);
